...
const LandingPageTemplate = ({ page }: LandingPageTemplateProps) => {
  ...
  const heroAltText = `${service.name}${location ? ` in ${location.city}` : ""}${industry ? ` for ${industry.name}` : ""} - Avorria Digital`;

  return (
    <div className="min-h-screen">
      {/* Hero Section with City/Industry Background */}
      <section className="relative min-h-[85vh] flex items-center -mt-20 pt-20 overflow-hidden">
        {/* Background Image using Next.js Image for SEO Alts */}
        <div className="absolute inset-0 z-0">
          {(heroImage || (location?.slug && getCityImage(location.slug))) ? (
            <Image
              src={heroImage || getCityImage(location!.slug)!}
              alt={heroAltText}
              fill
              priority
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-black/75 via-black/60 to-[hsl(220,25%,8%)] z-10" />
        </div>
